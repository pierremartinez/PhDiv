...
pidx <- grep("^patient id: ", series[10,])
spats <- gsub("^patient id: ", "", series[10,pidx])
tnm <- unlist(lapply(unique(spats), function(p){series[15,pidx[which(spats==p)]][1]}))
tstage <- as.numeric(gsub("tnm: T(\\d).+$", "\\1", tnm, perl=T))
nstage <- as.numeric(gsub("tnm: T\\d.*N(\\d)M.+$", "\\1", tnm, perl=T))
tstage2 <- tstage
tstage2[nstage>0] <- tstage2[nstage>0] + 0.5
names(tstage) <- names(tstage2) <- names(nstage) <- names(tnm) <- unique(spats)
...
save(distList, file=sprintf("../external_data/distList_li_%d.RData", nbest))
save(bList, file=sprintf("../external_data/bList_li_%d.RData", nbest))
load(sprintf("../external_data/distList_li_%d.RData", nbest))
load(sprintf("../external_data/bList_li_%d.RData", nbest))
...
pdf(sprintf("../plots/li_distances_%d.pdf", nbest))
heatmap.2(clMat[,patgrpo], Colv=F, col=bluered(100), trace="none", labCol=NA, dendrogram="row",
          labRow=paste("CL", 1:nbest, sep=""), mar=c(1,9), cexRow=1, cexCol=1)
...
for (i in rownames(nscore)) {
  ...
  nscore[i,] <- tmp / max(tmp, na.rm=T)
}

nclMat <- apply(nscore, 2, clusterise, cl=clbest, n=nbest)
...
pdf(sprintf("../plots/li_normal_distances_%d.pdf", nbest))
heatmap.2(clMat[,npatgrpo], Colv=F, col=bluered(100), trace="none", labCol=NA, dendrogram="row",
          labRow=paste("CL", 1:nbest, sep=""), mar=c(1,9), cexRow=1, cexCol=1)
par(mar=c(.5,.5,.5,.5), mfrow=c(2,1))
